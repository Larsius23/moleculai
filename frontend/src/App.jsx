  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        {error && <div className="bg-red-900 border border-red-600 text-red-300 rounded-xl px-5 py-4 text-sm">{error}</div>}
        {isLoading && <div className="text-center py-16 text-slate-400 text-sm animate-pulse">Analyzing molecule...</div>}
        {result && !isLoading && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-1"><MoleculeViewer imageBase64={result.image_base64} formula={result.formula} smiles={result.smiles} /></div>
              <div className="md:col-span-2"><LipinskiPanel analysis={result.lipinski_analysis} lipinskiPass={result.lipinski_pass} qedScore={result.qed_score} /></div>
            </div>
            <PropertiesPanel data={result} />
          </>
        )}
        {!result && !isLoading && !error && (
          <div className="text-center py-20 text-slate-500 text-sm">Enter a SMILES string or pick an example molecule to get started.</div>
        )}
      </main>
    </div>
  );
